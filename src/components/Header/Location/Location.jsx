...
import st from "./location.m.css";
import { useSelector } from 'react-redux';

const Location = () => {
  const location = useSelector(state => state.mainPage.location)

  return (<div className={st.location}>
      <svg className={st.locationIcon} enableBackground="new 0 0 512 512" height="512" viewBox="0 0 512 512"
      ...
      <span className={st.cityText}>{location}</span>
    </div>
  )
};
